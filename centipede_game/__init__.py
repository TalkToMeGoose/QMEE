...
class C(BaseConstants):
    ...
    NUM_ROUNDS = 3  # starting with 1 for now to be simple
...
class Subsession(BaseSubsession):  # for initialization and group separation
    # %|$ might be confusing to use round as a var
    # set initial round and turn numbers
    round_data = models.IntegerField(initial=1)
...
def creating_session(subsession):
    ...
    for x in range(1, C.NUM_TURNS_TOTAL+1):
        ...
        subsession.in_round(x).round_data = np.ceil(x/C.NUM_TURNS_PER_ROUND)
